...
# Voice cloning: Coqui XTTS v2 — free, local, zero-shot cloning from a short
# reference clip. The original "TTS" package is archived; install the
# maintained community fork, which still imports as `TTS`:
#     pip install coqui-tts
# First use downloads the multilingual model (~1.8GB) and synthesis is slow
# on CPU (several seconds per sentence) — normal, not a bug.
try:
...
# Reference clip for voice cloning — keep jarvis_voice_reference.wav next to
# this script. Longer/cleaner samples (15-30s) clone more accurately than a
# short one, but a few seconds of clean speech is enough to work.
VOICE_REFERENCE_FILE = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "jarvis_voice_reference.wav"
)
VOICE_CLONE_ACTIVE = HAS_XTTS and os.path.exists(VOICE_REFERENCE_FILE)
...
def _get_xtts_model():
    """Lazily load the XTTS model on first use (avoids the multi-second load
    delaying app startup) and reuse it after that."""
    global _xtts_model
    if _xtts_model is None:
        with _xtts_lock:
            if _xtts_model is None:
                os.environ.setdefault("COQUI_TOS_AGREED", "1")
                ui_queue.put(("LOG", "Loading voice-clone model (first run may take a while)..."))
                _xtts_model = CoquiTTS("tts_models/multilingual/multi-dataset/xtts_v2")
                ui_queue.put(("LOG", "Voice-clone model ready."))
    return _xtts_model
...
def speak_jarvis(text: str):
    ...
    try:
        # --- Try the cloned voice first ---
        if VOICE_CLONE_ACTIVE:
            wav_filename = f"jarvis_voice_{uuid.uuid4().hex[:10]}.wav"
            try:
                model = _get_xtts_model()
                model.tts_to_file(
                    text=text,
                    speaker_wav=VOICE_REFERENCE_FILE,
                    language="en",
                ...
                print(f"[XTTS Error]: {e} — falling back to default voice")
                ui_queue.put(("LOG", f"Voice clone failed ({e}), using default voice this time."))

        # --- Fallback: stock edge-tts British voice ---
        if not used_clone:
            async def _generate():
...
if __name__ == "__main__":
    ...
    if VOICE_CLONE_ACTIVE:
        print(f"[i] Voice cloning ACTIVE — speaking with {VOICE_REFERENCE_FILE}")
    elif HAS_XTTS:
        print(f"[!] coqui-tts installed but no reference file at {VOICE_REFERENCE_FILE} "
              "→ using default British voice")
    else:
        print("[!] Voice cloning inactive (pip install coqui-tts) → using default British voice")
